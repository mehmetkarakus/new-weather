document.addEventListener("DOMContentLoaded", () => {
  ...
  function dailyCard(day, dayData) {
    ...
    weatherBackground(dayData[0].weather.main);
    console.log(dayData);
  }
  ...
  const lowercaseJSONArray = JSONArray.map((city) => city.toLowerCase());

  function handleSearch() {
    const searchInputValue = searchInput.value.trim().toLowerCase();

    if (lowercaseJSONArray.includes(searchInputValue)) {
      fetchDataAndDisplay(searchInputValue);
  ...
  async function fetchDataAndDisplay(city) {
    try {
      const data = await fetchData(city);
  ...
  fiveDailyWeather.addEventListener("click", (event) => {
